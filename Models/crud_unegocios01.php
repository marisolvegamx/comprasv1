...
#vistaservicios
	...
	public function vistaUnegocioModel($init=false, $page_size=false, $cta, $tabla){
		//$stmt = Conexion::conectar()-> prepare("SELECT une_id, une_descripcion, 
//une_dir_pais, une_dir_ciudad, une_idcuenta,une_num_unico_distintivo, est_nombre FROM $tabla 
...
//where cue_clavecuenta=:cta order by une_dir_estado limit :init, :size  ");
       ...
       // $stmt->bindParam(":init", $init, PDO::PARAM_INT);
        ...
        $stmt = Conexion::conectar()-> prepare("SELECT tie_descripcion, tie_idpais, tie_idciudad from ca_tienda");
        $stmt->execute();
...
//INSERT INTO `ca_unegocios` (`une_id`, `une_descripcion`, `une_tipotienda`, `une_dir_calle`, `une_dir_numeroext`, `une_dir_numeroint`, `une_dir_manzana`, `une_dir_lote`, `une_dir_colonia`, `une_dir_delegacion`, `une_dir_municipio`, `une_dir_estado`, `une_dir_cp`, `une_dir_referencia`, `une_cla_pais`, `une_cla_ciudad`, `une_estatus`,`une_coordenadasxy`, `une_puntocardinal`) VALUES (2, 'KFC INSURGENTES TIJUANA', 1, 'BLVD. INSURGENTES ', '1809', '19', '3', '1', 'TERCERA ETAPA RIO TIJUANA', 'TIJUANA', 'TIJUANA', 'BAJA CALIFORNIA', '22560', '', 1, 1, 5, '', 1), (3, 'KFC NUEVO MEXICALI', 1, 'CALZ. LAZARO CARDENAS ', '300', '', '', '', 'NUEVA MEXICALI', 'MEXICALI', 'MEXICALI', 'BAJA CALIFORNIA', '21399', '', 2, 1, 1, '10011.1', 1);
